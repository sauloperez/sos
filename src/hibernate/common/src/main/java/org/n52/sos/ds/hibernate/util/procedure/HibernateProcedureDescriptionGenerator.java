...
import static org.n52.sos.ds.hibernate.dao.HibernateSqlQueryConstants.OBSERVABLE_PROPERTY;
import static org.n52.sos.ds.hibernate.dao.HibernateSqlQueryConstants.OFFERING;
import static org.n52.sos.ds.hibernate.dao.HibernateSqlQueryConstants.PROCEDURE;
...
public class HibernateProcedureDescriptionGenerator {
    private static final Logger LOGGER = LoggerFactory
            .getLogger(HibernateProcedureDescriptionGenerator.class);
    private static final String SQL_QUERY_GET_UNIT_FOR_OBSERVABLE_PROPERTY =
            "getUnitForObservableProperty";
    private static final String SQL_QUERY_GET_UNIT_FOR_OBSERVABLE_PROPERTY_PROCEDURE =
            "getUnitForObservablePropertyProcedure";
    private static final String SQL_QUERY_GET_UNIT_FOR_OBSERVABLE_PROPERTY_PROCEDURE_OFFERING =
     ...
     */
    public SensorML generateProcedureDescription(
            Procedure procedure, Session session) throws OwsExceptionReport {
     ...
     *                            If an error occurs
     */
    private ProcessModel createSmlProcessModel(Procedure procedure,
                                               Session session)
            throws OwsExceptionReport {
        ...
        String[] observableProperties =
                getObservablePropertiesForProcedure(procedure.getIdentifier());
     ...
     */
    private ProcessMethod createMethod(Procedure procedure,
                                       String[] observableProperties) {
     ...
     * @throws OwsExceptionReport If an error occurs
     */
    private System createSmlSystem(Procedure procedure, Session session)
            throws OwsExceptionReport {
     ...
     */
    private RulesDefinition createRulesDefinition(Procedure procedure,
                                                  String[] observableProperties) {
     ...
     *
     * @param procedure        Hibernate procedure entity
     * @param abstractSensorML SensorML process
     *
     * @throws OwsExceptionReport If an error occurs
     */
    private void setCommonValues(Procedure procedure,
     ...
     * @throws OwsExceptionReport If an error occurs
     */
    private List<SmlIo<?>> createOutputs(Procedure procedure,
                                         String[] observableProperties,
                                         Session session) throws
            OwsExceptionReport {
        try {
            final List<SmlIo<?>> outputs = Lists
                    .newArrayListWithExpectedSize(observableProperties.length);
            int i = 1;
            final boolean supportsObservationConstellation =
                    HibernateHelper
                    .isEntitySupported(ObservationConstellation.class, session);
            for (String observableProperty : observableProperties) {
                final SmlIo<?> output;
                if (supportsObservationConstellation) {
                    output = createOutputFromObservationConstellation(
                            procedure.getIdentifier(), observableProperty, session);
                } else {
                    output = createOutputFromExampleObservation(
                            procedure.getIdentifier(), observableProperty, session);
                }
                if (output != null) {
    ...
    private SmlIo<?> createOutputFromObservationConstellation(String procedure,
    ...
    private String queryUnit(ObservationConstellation oc, Session session) {
        ...
        } else if (HibernateHelper.isNamedQuerySupported(SQL_QUERY_GET_UNIT_FOR_OBSERVABLE_PROPERTY_PROCEDURE, session)) {
     ...
     *
     * @param clazz Name of not supported class
     */
    ...
    private SmlIo<?> createOutputFromExampleObservation(String procedure,
                                                        String observableProperty,
                                                        Session session)
            throws OwsExceptionReport {
        final Observation exampleObservation = getExampleObservation(
    ...
    private SmlPosition createPosition(Procedure procedure) {
        ...
        if (procedure.isSetLongLat()) {
            ...
            position.setPosition(createCoordinatesForPosition(
    ...
    private List<SweCoordinate<?>> createCoordinatesForPosition(
        ...
        return Lists.<SweCoordinate<?>>newArrayList(
                ...
                new SweCoordinate<Double>(SweCoordinateName.altitude.name(), zq));
    }
    ...
    private SweQuantity createSweQuantity(Object value, String axis, String uom) {
        return new SweQuantity().setAxisID(axis).setUom(uom).setValue(JavaHelper
                .asDouble(value));
    }

    private List<String> createDescriptions(Procedure procedure,
                                            String[] observableProperties) {
     ...
     * @param observableProperty ObservableProperty identifier
     * @param session            the session
     *
     ...
     */
    @VisibleForTesting
    Observation getExampleObservation(String identifier,
                                      String observableProperty,
        ...
        c.createCriteria(Observation.OBSERVABLE_PROPERTY)
                .add(Restrictions
                .eq(ObservableProperty.IDENTIFIER, observableProperty));
        c.createCriteria(Observation.PROCEDURE)
                .add(Restrictions.eq(Procedure.IDENTIFIER, identifier));
        c.setMaxResults(1);
        LOGGER
                .debug("QUERY getExampleObservation(identifier, observableProperty): {}",
                       HibernateHelper.getSqlString(c));
        final Observation example = (Observation) c.uniqueResult();
        if (example == null) {
            LOGGER
                    .debug("Could not receive example observation from database for procedure '{}' observing property '{}'.",
    ...
    String[] getObservablePropertiesForProcedure(String identifier) {
        Set<String> props = getCache()
                .getObservablePropertiesForProcedure(identifier);
